    ...
    return (
            <form onSubmit={e => e.preventDefault()}>
                <div className="container">
                    ...
                    {errors.obstaclesOfPreparationInTerm && <label className="text-danger"> {errors.obstaclesOfPreparationInTerm.message} </label>}
                    <input
                    ...
                    <ol>
                        {protocolAttachmentsDto.length > 0 && protocolAttachmentsDto.map((item, index) =>{
                            return <li key={index}>{item.attachmentName} <button className="btn-light" onClick={() => removeAttachment(index)}>Usuń z listy</button></li>
                        })
                        }
                    </ol>
